// var onionPrice = 43;
// var eggPrice = 10;
// var eggQuantity = 7;
// var totalPrice = onionPrice + eggPrice;
// var priceDifference = onionPrice - eggPrice;
// var eggPriceMultiplication = eggPrice * eggQuantity;

// // console.log(onionPrice);
// // console.log(eggPrice);
// // console.log(onionPrice + eggPrice);
// // console.log(totalPrice);
// // console.log(priceDifference);
// // console.log(eggPriceMultiplication);
/*--------------------------------------*/
/*----amar-practic start----*/
var onionPrice = 40;
var egePrice = 10;
var egeQuantity = 7;

console.log(onionPrice);
console.log(egePrice);
console.log(totalPrice);
var totalPrice = onionPrice + egePrice;
console.log(onionPrice + egePrice);

var priceDifference = onionPrice - egePrice;

var egePriceMultiplecation = egeQuantity * egePrice;

console.log(priceDifference);
console.log(egePriceMultiplecation);


var mango = 100;
var smallMango = 5;
var divaided = mango / smallMango;
console.log(divaided);
/*----amar-practic start-end---*/



// var moneyAvailable = 100;
// var orangePrice = 5;
// var orangeQuantity = moneyAvailable / orangePrice;
// // console.log(orangeQuantity);

// // special edition
// var firstNumber = 0.1;
// var secondNumber = 0.2;
// var total = firstNumber + secondNumber;
// total = total.toFixed(1);
// total = parseFloat(total);
// // console.log(total);

// // modulus
// var mangoes = 57;
// var hungryPerson = 12;
// var dividing = mangoes / hungryPerson;
// console.log(dividing);
// var remaining = mangoes % hungryPerson;
// console.log(remaining);